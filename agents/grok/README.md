...
xAI has two agent surfaces. On Linux the official install is the Grok Build CLI (`grok`). The Grok Bot desktop app has no Linux build.

Sources: [x.ai/cli](https://x.ai/cli), [xai-org/grok-build](https://github.com/xai-org/grok-build), [docs.x.ai/build](https://docs.x.ai/build/overview), [Grok Bot get started](https://docs.x.ai/grok-bot/get-started).
...
[Grok Bot](https://x.ai/bot) is a separate product — desktop app for macOS/Windows (+ iOS), no Linux build. It needs an eligible paid plan (SuperGrok Plus, SuperGrok Heavy, Cursor Pro+/Ultra, Cursor Teams Standard/Premium) and a Cursor account. Not installable on Linux; the CLI above is the Linux option.
